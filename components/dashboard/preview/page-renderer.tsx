...
function WeatherIcon({
  ...
  const iconName = useMemo(() => getWeatherIconName(condition), [condition]);
...
export function PreviewPageRenderer({
  ...
  now,
  pageIndex,
}: PreviewPageRendererProps) {
  const pageEntity = getBoundEntityState(activePage, homeAssistantStates);
  ...
  if (activePage.type === "weather-focus") {
    return (
  ...
  if (activePage.type === "media-player") {
    return (
    ...
    <div className="space-y-2 overflow-hidden">
      {activePage.widgets.map((widget, index) => {
        ...
        switch (widget.type) {
          ...
          case "title":
            return <PreviewTitleSeparator key={widget.id} widget={widget} />;
          default:
            return null;
        }
      })}
